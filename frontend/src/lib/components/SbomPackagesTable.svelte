...
<div class="border border-line-hairline rounded-sm overflow-x-auto bg-surface-panel font-mono text-[11px]">
  <div class="grid min-w-[860px] grid-cols-[minmax(180px,1.4fr)_120px_110px_100px_minmax(150px,1fr)_110px] gap-3 px-3 py-2 bg-surface-inset border-b border-line-hairline text-[10px] text-ink-muted items-center">
    <div>Package</div>
    <div>Version</div>
    <div>Ecosystem</div>
    <div>Type</div>
    <div>Licence</div>
    <div class="text-right">Vulnerabilities</div>
  </div>
  {#each packages as item (item.bom_ref ?? `${item.name}:${item.version ?? ''}`)}
    <div class="grid min-w-[860px] grid-cols-[minmax(180px,1.4fr)_120px_110px_100px_minmax(150px,1fr)_110px] gap-3 px-3 py-2 border-b border-line-hairline last:border-b-0 items-center">
      ...
      <div class="text-right tabular-nums" class:text-state-failed={(item.vulnerability_count ?? 0) > 0} class:text-ink-muted={(item.vulnerability_count ?? 0) === 0}>
...
<p class="mt-2 text-[11px] text-ink-muted">
  A dash means the inventory does not embed vulnerability analysis. Dependency vulnerabilities remain available under Findings.
</p>
