...
async def resolve_repo_token(
    ...
    if owner and owner != settings.github_org and settings.token_encryption_key:
        ...
        from app.secrets import decrypt as _decrypt
...
async def set_user_role(
    ...
    from app.api.deps_roles import MUTABLE_ROLES
    from app.infrastructure.db.models import User
        ...
        if not has_stub:
            raise HTTPException(
                ...
                detail=(
                    ...
                    "(backend/resources/templates/assurance-scan.yml) to the repo's "
        ...
        if exc.code == 404:
            raise HTTPException(
                ...
                detail=(
                    ...
                    "(backend/resources/templates/assurance-scan.yml) or register the organisation "
                    "in Settings."
                ),
            ) from exc
        if exc.code == 422:
...
async def preview_mcp_token(
    ...
    # Starlette's directly constructed test Request can expose byte-valued
    # URL components even though the public URL type annotates them as text.
