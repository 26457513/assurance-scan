...
if [ -n "${ASSURANCE_SCAN_COMPOSE_FILE:-}" ]; then
  COMPOSE_FILE="$ASSURANCE_SCAN_COMPOSE_FILE"
elif [ -f "$BACKEND_ROOT/docker-compose.security.yml" ]; then
  COMPOSE_FILE="$BACKEND_ROOT/docker-compose.security.yml"
else
  COMPOSE_FILE="$BACKEND_ROOT/../docker-compose.security.yml"
fi
...
run_prefetch_services() {
  ...
  for svc in "${services[@]}"; do
    ...
    if [ -n "$log_file" ]; then
      $COMPOSE_BIN -f "$COMPOSE_FILE" --profile prefetch run --rm "$svc" >> "$log_file" 2>&1
    else
      $COMPOSE_BIN -f "$COMPOSE_FILE" --profile prefetch run --rm "$svc" >/tmp/asvs-prefetch-"$svc"-$$.log 2>&1
    fi
    rc=$?
    if [ "$rc" -eq 0 ]; then
      ok "$label"
...
REPORT_DIR="$BACKEND_ROOT/reports/$RUN_ID"
...
if "$SCRIPT_DIR/preflight.sh" >> "$REPORT_DIR/run.log" 2>&1; then
  ok "environment"
else
...
if prefetch_needs_refresh "scanner-images"; then
  ...
  if $COMPOSE_BIN -f "$COMPOSE_FILE" pull --quiet >> "$REPORT_DIR/run.log" 2>&1; then
    ok "scanner images"
...
run_scanner_job() {
  ...
  (
    ...
    started_at="$(date +%s)"
    $COMPOSE_BIN -f "$COMPOSE_FILE" run --rm "$service" >> "$job_log" 2>&1
    rc=$?
    ...
    if [ -n "$out_file" ] && [ -n "$SCANNER_IGNORE_ARG" ]; then
      python3 "$SCRIPT_DIR/apply-scannerignore.py" \
        $SCANNER_IGNORE_ARG \
        --scanner "$service" \
...
python3 "$SCRIPT_DIR/manual-evidence-template.py" \
...
step_started_at="$(date +%s)"
if python3 "$SCRIPT_DIR/discover-project-tests.py" \
  --target-dir "$TARGET_DIR" \
...
if python3 "$SCRIPT_DIR/generate-assurance-test-pack.py" "${ASSURANCE_PACK_ARGS[@]}" >> "$REPORT_DIR/run.log" 2>&1; then
...
python3 "$SCRIPT_DIR/generate-evidence-bundle.py" "${BUNDLE_ARGS[@]}" >> "$REPORT_DIR/run.log" 2>&1
...
python3 "$SCRIPT_DIR/generate-agent-prompt.py" "${PROMPT_ARGS[@]}" >> "$REPORT_DIR/run.log" 2>&1
...
step_started_at="$(date +%s)"
if python3 "$SCRIPT_DIR/validate-report-artifacts.py" --report-dir "$REPORT_DIR" >> "$REPORT_DIR/run.log" 2>&1; then
...
if [ "${ASSURANCE_SCAN_PUBLISH_FINDINGS:-0}" = "1" ] && [ -n "${ASSURANCE_SCAN_PUBLISH_FINDINGS_DIR:-}" ]; then
  ...
  if python3 "$SCRIPT_DIR/publish_findings.py" "${PUBLISH_ARGS[@]}" >> "$REPORT_DIR/run.log" 2>&1; then
