...
def count_clamav(path: Path) -> int:
    ...
    try:
        ...
        return max(0, len([line for line in text.splitlines() if "FOUND" in line]))
    except Exception:
        return 0
...
SCHEMA_REFS = {
    "assurance_test_pack": "resources/schemas/assurance-test-pack.schema.json",
    "evidence_bundle": "resources/schemas/evidence-bundle.schema.json",
...
def main() -> int:
    ...
    if release_recommendation != "READY":
        ...
        lines.append("  - Validate audit logging, run pen-test, etc. (see manual-evidence-required.md)")
    lines.append("=" * 70)
