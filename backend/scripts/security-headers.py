...
def check_url(url: str, timeout: int = 15) -> dict:
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.hostname:
        return {
            "url": url,
            "status_code": None,
            "error": "URL must use HTTP or HTTPS and include a hostname",
            "findings": [],
        }
    ...
    try:
        # The scanner intentionally accepts a user-selected local or remote web
        # target; validation above constrains it to HTTP(S) URLs with a host.
