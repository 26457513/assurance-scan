...
def _lineage_tailoring(
    decision: dict[str, Any],
    *,
    target: str,
    source_id: str,
) -> list[dict[str, Any]]:
    """Project reviewed edits into the stricter artifact-lineage shape."""
    ...
    for item in _tailoring_for(decision, target=target, source_id=source_id):
        ...
        entry: dict[str, Any] = {
            "field": str(field),
            "rationale": str(item.get("rationale") or decision.get("reason") or "Reviewed blueprint tailoring."),
        }
        if "from" in item:
            entry["from"] = deepcopy(item["from"])
        elif field == "id":
            entry["from"] = source_id
        if "to" in item:
            entry["to"] = deepcopy(item["to"])
