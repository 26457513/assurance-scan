...
ALLOWED_AUTHORITY_HOSTS = frozenset({"github.com", "raw.githubusercontent.com"})
...
def fetch(url: str, path: Path) -> None:
    ...
    parsed = urlsplit(url)
    if parsed.scheme != "https" or parsed.hostname not in ALLOWED_AUTHORITY_HOSTS:
        raise SystemExit(f"Refusing unapproved authority URL: {url}")
    if parsed.username is not None or parsed.password is not None:
        raise SystemExit("Authority URL must not contain credentials")
    path.parent.mkdir(parents=True, exist_ok=True)
    request = urllib.request.Request(url, headers={"User-Agent": "assurance-scan-authority-sync/1"})
    # URLs originate in the reviewed authority registry and are constrained to
    # the explicit HTTPS host allowlist above.
...
def main() -> int:
    ...
    parser.add_argument("--registry", type=Path, default=REPO_ROOT / "resources" / "authority-sources" / "rulesets.json")
