...
from scanner_parsers import (
    ...
    SEVERITY_COLORS,
    SEVERITY_ORDER,
    actionable_finding_total,
    aggregate_severity_strict,
    evidence_markup,
    finding_markup,
    finding_total,
    fmt_bytes,
    ...
    md_to_html,
    output_family,
    parse_ignored_from_log,
    scan_surface_groups,
    scanner_finding_value,
    scanner_output_link,
    secret_breakdowns,
    sev_badge,
    status_pill,
...
CONFIG_SCHEMA_BY_ROLE = {
    "fr_catalog": "resources/schemas/fr-catalog.schema.json",
    "compliance_regime": "resources/schemas/compliance-regime.schema.json",
    "compliance_mapping_pack": "resources/schemas/compliance-mapping-pack.schema.json",
    "scanner_compliance_mapping_pack": "resources/schemas/scanner-compliance-mapping-pack.schema.json",
    "assurance_framework": "resources/schemas/assurance-framework.schema.json",
    "assurance_instance": "resources/schemas/assurance-instance.schema.json",
}

PLANNING_SCHEMA_BY_ROLE = {
    "project_intake": "resources/schemas/project-intake.schema.json",
    "project_config_selection": "resources/schemas/project-config-selection.schema.json",
    "blueprint_selection_proposal": "resources/schemas/blueprint-selection-proposal.schema.json",
    "blueprint_decision_log": "resources/schemas/blueprint-decision-log.schema.json",
    "config_update_proposal": "resources/schemas/config-update-proposal.schema.json",
    "resolved_project_planning_contract": "resources/schemas/resolved-project-planning-contract.schema.json",
    "project_assurance_contract": "resources/schemas/project-assurance-contract.schema.json",
    "project_design_document_manifest": "resources/schemas/project-design-document-manifest.schema.json",
    "code_studio_handoff_pack": "resources/schemas/code-studio-handoff-pack.schema.json",
    "code_generator_handoff_pack": "resources/schemas/code-generator-handoff-pack.schema.json",
}
...
def _regime_metadata_candidates(regime: str) -> list[Path]:
    ...
    if not slug:
        return []
    root = REPO_ROOT / "resources" / "compliance-regimes" / slug
    if not root.exists():
        return []
...
def _all_compliance_regime_paths() -> list[Path]:
    root = REPO_ROOT / "resources" / "compliance-regimes"
    if not root.exists():
        return []
...
def _all_assurance_framework_paths() -> list[Path]:
    root = REPO_ROOT / "resources" / "assurance-frameworks"
    if not root.exists():
        return []
    return sorted(path for path in root.rglob("*.json") if path.is_file())
...
def _image_data_path(path: Path) -> str:
    try:
        ...
        return f"/opt/assurance-scan/backend/{relative.as_posix()}"
    except ValueError:
        return str(path)
...
def _framework_label(data: dict[str, Any]) -> str:
    ...
    if framework_id and title and title != framework_id:
        normalized_title = re.sub(r"^" + re.escape(framework_id) + r"\s*[-:]?\s*", "", title, flags=re.I)
        return f"{framework_id} - {normalized_title}"
    ...
    *,
    ...
    manifest = {
        ...
        "graph_builder": {
            ...
            "version": 1,
            "vocabulary": "resources/schemas/defs.schema.json",
        },
        "toolchain": toolchain,
        "graph": {
...
def render_coverage(evidence: dict, report_dir: Path, evidence_view: dict[str, Any] | None = None) -> str:
    ...
    out.append(
        ...
        '<button type="button" class="file-page-tab active" data-file-tab="config" role="tab" aria-selected="true">Runtime config files</button>'
        '<button type="button" class="file-page-tab" data-file-tab="evidence" role="tab" aria-selected="false">Evidence</button>'
        '</div>'
    )
    if config_html:
...
def render_assurance_tests_page(report_dir: Path) -> str:
    ...
    len([item for item in queue if item.get("source") == "generated"])
    len([item for item in queue if item.get("source") == "planned_tbt"])
    len([item for item in queue if not item.get("tbt") and item.get("native_path")])
    len([item for item in queue if item.get("safety") == "review_required"])
    len([item for item in queue if item.get("source") == "planned_tbt"])
    ...
    for idx, item in enumerate(queue):
        ...
        [fr_by_id.get(fr_id, {}).get("title", fr_id) for fr_id in fr_ids]
        ...
        html.escape(str(item.get("pack_id") or tbt_id or decision_id))
        type_notes = {
...
def render_instructions_page(report_dir: Path, evidence: dict) -> str:
    ...
    report_dir / "compliance-mapping-pack.snapshot.json"
    ...
    shell_quote(fr_catalog)
    shell_quote(junit_path)
    ...
    scan_config_flags = [
        ...
        "  --scanner-compliance-mapping-pack '/opt/assurance-scan/backend/resources/scanner-mappings'",
    ]
    ...
    sync_authority_command = "\n".join([
        ...
        f"{shell_quote(str(REPO_ROOT / 'resources' / 'authority-sources' / 'rulesets.json'))} --strict",
    ])
    ...
    config_review_command = "\n".join([
        ...
        "  --blueprint '/opt/assurance-scan/backend/resources/blueprints/security-core/asvs-5.0.0/fr-catalog.blueprint.json' " + chr(92),
    ...
    blueprint_catalog_q = shell_quote("/opt/assurance-scan/backend/resources/blueprints/security-core/asvs-5.0.0/fr-catalog.blueprint.json")
    blueprint_mapping_pack_q = shell_quote("/opt/assurance-scan/backend/resources/blueprint-mappings/security-core/asvs/5.0.0.json")
    blueprint_nist_mapping_pack_q = shell_quote("/opt/assurance-scan/backend/resources/blueprint-mappings/security-core/nist-800-53/5.2.0.json")
    docker_cli_base = "\n".join([
        "docker run --rm -it \\",
        mount_flags,
        f"  -w {workdir_expr} \\",
...
def _richest_framework_path(report_dir: Path) -> str | None:
    ...
    candidates = [
        report_dir / "assurance-framework.snapshot.json",
        repo_root / "resources" / "assurance-frameworks" / "jsp-453" / "1.0.0-draft.json",
    ]
    for parent in [report_dir, *report_dir.parents]:
        if parent.name == "runtime":
            candidates.extend([
                parent / "resources" / "assurance-frameworks" / "jsp-453" / "1.0.0-draft.json",
                parent / "jsp-453.assurance-framework.draft.json",
                parent / "resources" / "fixtures" / "target-schemas" / "assurance-framework.example.json",
            ])
            break
    candidates.append(repo_root / "resources" / "fixtures" / "target-schemas" / "assurance-framework.example.json")
...
def _default_scanner_compliance_mapping_paths(report_dir: Path) -> list[str]:
    ...
    for candidate in (
        report_dir / "scanner-compliance-mapping-packs",
        Path(__file__).resolve().parent.parent / "resources" / "scanner-mappings",
    ):
        if candidate.exists():
...
def render(*, report_dir: Path, fr_catalog_path: str | None = None,
    ...
    C["pass"] if rec == "READY" else C["fail"]
    assurance.get("failed", sum(1 for info in scanner_health.values() if info.get("status") == "FAIL"))
    assurance.get("warned", sum(1 for info in scanner_health.values() if info.get("status") == "WARN"))
    assurance.get("skipped", sum(1 for info in scanner_health.values() if info.get("status") == "SKIPPED"))
    ...
    assurance.get("automated_assurance_pct", 0)
    assurance.get("asvs_traceability_pct", 0)
