...
DEFAULT_BLUEPRINT = REPO_ROOT / "resources" / "blueprints" / "security-core" / "asvs-5.0.0" / "fr-catalog.blueprint.json"
...
def main() -> int:
    ...
    try:
        ...
        if not fr_catalog_path.exists():
            ...
            existing_catalog = json.loads(fr_catalog_path.read_text())
            # An empty bootstrap has no IDs to collision-check and cannot satisfy
            # the final catalogue schema until the reviewed additions are applied.
            # Treat it like a newly created bootstrap, while still validating the
            # fully populated output in apply-config-update-proposal.py.
            fresh_catalog = not (
                existing_catalog.get("frs") or existing_catalog.get("tbts")
            )
