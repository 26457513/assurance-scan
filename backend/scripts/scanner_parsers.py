...
def gauge_chart(value_pct: int, *, color: str, label: str, subtitle: str = "", size: int = 200) -> str:
    ...
    subtitle_html = f'<div class="gauge-subtitle">{html.escape(subtitle)}</div>' if subtitle else ""
    return (
...
def md_to_html(md: str) -> str:
    ...
    for line in lines:
        ...
        if stripped.startswith("```"):
            if in_code:
                out.append('<pre class="md-code"><code>' + "\n".join(html.escape(line) for line in code_buf) + "</code></pre>")
                code_buf = []
                in_code = False
            else:
                close_list()
...
def finding_markup(value) -> str:
    if isinstance(value, dict):
        ...
        chips_html = "".join(chips) if chips else '<span class="plain-count">clean</span>'
        return f'<div class="finding-cells">{chips_html}</div>'
    if isinstance(value, int):
