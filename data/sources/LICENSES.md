...
- **Snapshot file:** `asvs_requirements.json`
...
Each snapshot is regenerated by `scripts/build-mapping-sources.py`. The script records fetch date and source commit SHA in each JSON's `meta` block so reviewers can verify currency.
