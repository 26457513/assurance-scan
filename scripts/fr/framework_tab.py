...
def _render_resolved_assurance_detail(resolved: dict | None) -> str:
    ...
    return (
        ...
        f'{"<ul class=\"fw-culprit-list\" role=\"list\">" + reason_items + "</ul>" if reason_items else ""}'
...
def _render_resolved_scanner_evidence(scanner_evidence: list[dict], blockers: list[dict]) -> str:
    ...
    for signal in evidence[:8]:
        ...
        items.append(
            ...
            f'{f"<span>{html.escape(short_text(str(message), 120))}</span>" if message else ""}'
            f'{f"<div class=\"manual-evidence\">Mapping: {html.escape(str(mapping))}</div>" if mapping else ""}'
            f'{f"<div class=\"manual-evidence\">Effect: {html.escape(str(effect))}; strength: {html.escape(str(strength))}</div>" if effect or strength else ""}'
