...
def parse_expectations(raw_values: list[str]) -> dict[str, tuple[str, str]]:
    expectations: dict[str, tuple[str, str]] = {}
    for raw in raw_values:
        if "=" not in raw or "/" not in raw:
            raise ValueError("expected --expect-relationship RULESET=relationship/strength")
        ruleset, expected = raw.split("=", 1)
        relationship, strength = expected.split("/", 1)
        expectations[ruleset] = (relationship, strength)
    return expectations
...
def coverage_for_pack(
    *,
    ...
    for mapping in pack.get("mappings") or []:
        ...
        if expected and (relationship, strength) != expected:
            errors.append(
                f"{pack_path}: {mapping_id} expected {expected[0]}/{expected[1]} "
